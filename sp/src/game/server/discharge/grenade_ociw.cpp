...
#define ociw_GRENADE_MAX_DANGER_RADIUS	300
...
void CGrenadeociw::GrenadeociwThink( void )
{
	...
	if (!m_bIsLive)
	{
		...
		if (m_fSpawnTime + MAX_ociw_NO_COLLIDE_TIME < gpGlobals->curtime)
		{
			m_bIsLive  = true;
		}
	}
	...
	if( m_fDangerRadius <= ociw_GRENADE_MAX_DANGER_RADIUS )
	{
		m_fDangerRadius += ( ociw_GRENADE_MAX_DANGER_RADIUS * 0.05 );
	}
